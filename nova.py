...
def handle_runtime_result(orb, spine, result):
    ...
    if status == "handled":
        ...
        if category == "conversation":
            ...
            voice_engine = spine.get_voice_engine()
            if voice_engine is not None:
                voice_engine.speak(result.get("response", "I am checking the current situation."))
        elif category == "exit":
        ...
        QTimer.singleShot(900, lambda: orb.set_state("Listening", (0, 220, 120)))
    elif status == "empty":
...
def voice_wake_worker(spine, ui_queue, command_queue):
    ...
    while True:
        try:
            # Branch 2: Check V key press / hold
            v_key_pressed = False
            if key_monitor is not None and (key_monitor.was_pressed() or key_monitor.is_held()):
                v_key_pressed = True
